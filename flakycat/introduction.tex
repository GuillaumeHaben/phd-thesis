...
% FlakyCat relies on CodeBERT~\cite{feng2020codebert} to build appropriate code representations and allow static test flakiness predictions, i.e., predictions based solely on test code. 
...
% We applied data augmentation to end up with 964 samples, and then use the annotated dataset to train and evaluate FlakyCat, comparing it with relevant baselines.
...
\begin{itemize}
   ...
   % \textbf{Approach:} We compare FlakyCat with existing machine learning approaches adapted to the multi-class classification problem. \\
    \textbf{Findings:} Our results show that FlakyCat is capable of predicting flakiness categories with an F1 score of 73\%, outperforming classifiers based on traditional supervised machine learning. %that was used in previous works

    \item \textsc{\textbf{RQ2:}} How effective is FlakyCat at predicting each of the considered flakiness categories?\\
    %\textit{Motivation} We investigate more on the performance of our approach for each category. We wonder if the information embedded in the CodeBERT representation of test code can help classifying the different categories.\\
    ...
    %\textit{Motivation} An important step in many machine learning tasks is model interpretability. We want to check what information the model learnt.\\
