...
%Regression testing is a commonly adopted strategy by practitioners. 
...
%This assumption holds when tests are deterministic.
...
%A common approach to deal with flakiness is to rerun a failing test a number of times, hoping that it would pass at some point. These reruns imply costs both computation and time wise.
...
%Recently, some studies explored static approaches for predicting flaky tests in order to reduce the costs caused by reruns in dynamic approaches.
%King \etal~\cite{??} implemented a Bayesian Network to give a probability for a test to be flaky based on some of its characteristics. 
%Bertolino \etal~\cite{??} presented FLAST, a static approach which can flag tests as flaky based on their level of similarity with previously identified flaky tests. 
...
The original study (Pinto \etal) evaluated the performance of different machine learning models on different representations of the test code and found that the vocabulary of tests can predict test flakiness with 95\% of accuracy (F1 score). %Their evaluation showed that the vocabulary of tests can indeed be leveraged to predict flakiness.
...
%This means that flaky tests can be detected statically while avoiding reruns and all flakiness costs.
%it several times can be a valuable piece of information to avoid such costs.
...
%As the research works in the domain of flakiness are recent, there is still a need for more data in order to better understand the problem. The same applies for machine learning models. Existing datasets of known flaky tests are rare and small compared to datasets in other branches of software engineering research like for vulnerability prediction for example. DeFlaker and iDFlakies respectively identified and shared 1,874 and 422 flaky tests in 26 and 82 Java projects. Previously cited studies showed that learning from flaky tests to predict the status of new ones is feasible, but the generalisation of the concepts is yet to be proven. 
...
In this paper, we present a conceptual replication of the study of Pinto \etal~\cite{Pinto2020}. % that offers an interesting solution to predicting test flakiness. In particular, we seek to understand the conditions under which the approach holds when considering other configurations and subjects. 
...
%there is a growing body of literature in the domain of flakiness prediction; 
...
%The remainder of this paper is structured as follows: Section II presents the original study and Section III explains our replication methodology. Section IV reports on the study results, while Section V discusses the implications and threats of this study.
%We discuss related works in Section V and we conclude with perspectives in Section VI.
